                    ...
                    <tbody>
                        ...
                        foreach ($findCustomers as $findCustomerResults) {
                            ...
                            <tr>
                                ...
                                $FindTransactionsQuery = "SELECT ATT.voucherNo,ATT.toId AS ATOID,ATT.byId AS ABYID,ATO.firstName AS ATONAME,ABY.firstName AS ABYNAME,ATT.voucherDate,ATT.toAmount,ATT.byAmount,ATT.voucherType,ATT.remarks FROM account_transactions ATT INNER JOIN accounts ATO ON ATT.toId = ATO.Aid INNER JOIN accounts ABY ON ATT.byId = ABY.Aid WHERE (ATT.toId = '$CustomerId' OR  ATT.byId = '$CustomerId') AND ATT.hiddenStatus <> '1' AND ATT.voucherType IN ('PV','RV') AND  ATT.voucherDate BETWEEN '$BeforeDate' AND '$Afterdate'";
                                //echo $FindTransactionsQuery;
                                $FindTransactions = mysqli_query($con, $FindTransactionsQuery);
                                $NEWSUMDR = 0;
                                $NEWSUMCR = 0;
