...
<body>
        ...
        <div class="container mainContents">
            <div class="card card-body main_card mt-5 shadow-lg p-3 mb-5">
                ...
                <div class="main_heading d-flex justify-content-between mb-2 shadow p-2">
                    <div>
                        <h3 class="pt-3"> PORTAL RECHARGE </h3>
                    </div>
                </div>
                ...
                ?>
                    ...
                    <form id="PortalForm" class="AddForm px-5" novalidate>
                        <div class="row">
                            <div class="col-12 text-center">
                                <h5>PORTAL BALANCE - &#8377; <span class="portalBalance"></span> </h5>
                            </div>
                        </div>
                        <div class="row mt-3">
                            <div class="col-6">
                                <label for="re_to" class="TktcommonFont">To</label>
                                <select class="form-select" name="PortalTo" id="po_to" required readonly>
                                    <option selected value="">Portal</option>
                                </select>
                            </div>
                            <div class="col-6">
                                <label for="re_by" class="TktcommonFont">By</label>
                                <select class="form-select" name="PortalBy" id="po_by" required>
                                    <!-- <option hidden value="">Ledger</option> -->
                                    <?php
                                    $ReceiptTo = mysqli_query($con, "SELECT * FROM accounts WHERE accntGroupId = '17'");
                                    foreach ($ReceiptTo as $ReceiptToResult) {
                                        echo '
                                        <option value="' . $ReceiptToResult["Aid"] . '">' . $ReceiptToResult["firstName"] . ' ' . $ReceiptToResult["lastName"] . '</option>
                                        ';
                        ...
                        <div class="row mt-3">
                            <div class="col-6">
                                ...
                                <input type="number" class="form-control" name="PoAmount"  id="po_amount" required>
                            </div>
                            <div class="col-6">
                                <label for="re_amount" class="TktcommonFont">Portal Bonus</label>
                                <input type="number" class="form-control" name="PoBonus" id="po_bonus">
                            </div>
                        ...
                        <div class="row mt-3">
                            <div class="col-6">
                                ...
                                <input type="date" value="<?= date('Y-m-d'); ?>" class="form-control" name="PoDate" id="po_date" required>
                            </div>
                        </div>
    ...
    <script>
        ...
        $('#btnConfirm').click(function() {
            location.replace('PortalAdd.php');
        });


        function portalBalance() {
            var fetchBalance = 'fetch_data';
            $.ajax({
                type: "POST",
                ...
                success: function(data) {
                    console.log(data);
                    var BalanceResult = JSON.parse(data);
                    if (BalanceResult.Status == "1") {
                        //console.log((test).toLocaleString('en-IN'));
                        $('.portalBalance').text((Number(BalanceResult.PortalBalance)).toLocaleString('en-IN'));
                    }
                }
            });
        }
        ...
        $(function() {
            ...
            let validator = $('#PortalForm').jbvalidator({
            ...
            validator.validator.custom = function(el, event) {
                ...
                // if ($(el).is('#po_amount') && $(el).val() == 0) {
                //     return 'Cannot be Zero';
                // }
            }

            $(document).on('submit', '#PortalForm', (function(e) {
                e.preventDefault();
                var PortalData = new FormData(this);
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        $('#loading').hide();
                        console.log(data);
                        $('#PortalForm').removeClass("disable");
                        if (TestJson(data) == true) {
                            var response = JSON.parse(data);
                            if (response.addPortalRecharge == "0") {
